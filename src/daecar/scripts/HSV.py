...
#         cv2.createTrackbar('S Lower', 'HSV Tuner', 50, 255, self.update_hsv)
#         cv2.createTrackbar('S Upper', 'HSV Tuner', 255, 255, self.update_hsv)
#         cv2.createTrackbar('V Lower', 'HSV Tuner', 50, 255, self.update_hsv)
...
#             filtered_image = cv2.bitwise_and(self.current_image, self.current_image, mask=mask)

#             # 결과 출력
#             combined = np.hstack((self.current_image, filtered_image))
...
class HSVTuner:
    def __init__(self):
        ...
        # HSV 범위 초기값 설정
        self.hsv_lower = np.array([0, 30, 30])  # 초기값: H=0~179, S=30~255, V=30~255
        self.hsv_upper = np.array([179, 255, 255])

        # OpenCV 창 설정 및 트랙바 초기화
    ...
    def process_image(self):
        ...
        try:
            ...
            # 흰색 이미지 생성
            white_only = np.zeros_like(self.current_image)
            white_only[mask > 0] = [255, 255, 255]  # 흰색 영역 만들기

            # 결과 출력
            combined = np.hstack((self.current_image, white_only))
